      ...
      <template #label>
        <span v-if="!!field.options.labelIconClass" class="custom-label">
          ...
          <template v-else-if="field.options.labelIconPosition === 'rear'">
            <template v-if="!!field.options.labelTooltip">
              <span :style="{ color: labelFontColor, fontSize: labelFontSize + 'px' }">{{ label }}</span
              ><el-tooltip :content="field.options.labelTooltip" effect="light"> <svg-icon :icon-class="field.options.labelIconClass" /></el-tooltip
            ></template>
            <template v-else>
              <span :style="{ color: labelFontColor, fontSize: labelFontSize + 'px' }">{{ label }}</span
              ><svg-icon :icon-class="field.options.labelIconClass"
...
export default {
  ...
  computed: {
    ...
    labelFontSize() {
      return !!this.field.options.labelFontSize ? this.field.options.labelFontSize : this.formConfig.labelFontSize || 16;
    },
    labelFontColor() {
      return !!this.field.options.labelFontColor ? this.field.options.labelFontColor : this.formConfig.labelFontColor || "#333333";
    },
  },
  created() {
